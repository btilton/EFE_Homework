/*Program Example 7.9: Sets the mbed up for async communication, and exchanges data with a similar node, sending its own switch positions, and displaying those of the other. 
                                                                 */
#include "mbed.h"
Serial async_port(PTD3, PTD2);          //set up TX and RX on pins 9 and 10
DigitalOut red_led(LED_RED);             //red led
DigitalOut green_led(LED_GREEN);           //green led
DigitalOut strobe(PTD1);               //a strobe to trigger the scope
DigitalIn switch_ip1(PTD5);
DigitalIn switch_ip2(PTD0);
